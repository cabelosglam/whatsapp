...
def leads_page():
    ...
    for r in rows:
        ...
        telefone = safe_str(r.get("TELEFONE") or r.get("Telefone") or r.get("telefone"))
        if not telefone:
            continue

        stage = safe_str(r.get("STAGE") or "start").lower()
        # normaliza
        stage = stage.lower()
...
def listar_conversas():
    ...
    conversas = {}
    for r in rows:
        telefone = safe_str(r.get("TELEFONE") or r.get("Telefone"))
        if not telefone:
            continue
        stage = safe_str(r.get("STAGE") or "start").lower()
